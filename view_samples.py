...
def show_training_samples(
  ...
  render_mano=False,
  saveImage=False,
  save_path=dir_path("./SAVE_TRAININGS")
  ):
    if render_mano:
    ...
    db_data_annoList = [list(a) for a in db_data_anno]
    ...
    for idx in range(db_size('training')):
        ...
        K, mano, xyz = db_data_annoList[idx]
        ...
        ax2.axis('off')
        if saveImage:
          plt.savefig(f'{save_path}/TRAIN_{idx}.jpg')
        else:
          plt.show()


def show_eval_samples(
  base_path, 
  num2show=None,
  saveImage=False,
  save_path=dir_path("./SAVE_EVALUATION")
  ):
    if num2show == -1:
    ...
    for idx in  range(db_size('evaluation')):
        ...
        ax1.axis('off')
        if saveImage:
          plt.savefig(f'{save_path}/EVAL_{idx}.jpg')
        else:
          plt.show()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--safeImage',
                        action='store_true',
                        help="Save or not images")
    
    parser.add_argument('--save_path', 
                        type=dir_path,
                        default=dir_path("./SAVE_TRAININGS"),
                        help='Folder to save training images, default "./SAVE_TRAININGS".'
                        )
